...
void time_test2(int n = 5000) {
    ...
    double h = 1. / (n - 1);
    InterfaceMPI<Solver, Forwarding> interface(n, 1 / (h));
    double t = -omp_get_wtime();
...
int main(int argc, char** argv)
{
    ...
    if (rank == 0)
    {
        std::cout << "----------------- Jacoby iterations -----------------\n\n";
        std::cout << "Send and Recv: \n";
    }

    time_test2<JacobyIterationBlock, ForwardingTypes::SendAndRecv>();

    if (rank == 0)
        std::cout << "SendRecv: \n";

    time_test2<JacobyIterationBlock, ForwardingTypes::SendRecv>();

    if (rank == 0)
        std::cout << "ISend and IRecv: \n";

    time_test2<JacobyIterationBlock, ForwardingTypes::ISendAndIRecv>();

    if (rank == 0)
    {
        std::cout << "\n\n----------------- Red and Black iterations -----------------\n\n";
        std::cout << "Send and Recv: \n";
    }

    time_test2<RedNBlackIterationsBlock, ForwardingTypes::SendAndRecv>();

    if (rank == 0)
        std::cout << "SendRecv: \n";

    time_test2<RedNBlackIterationsBlock, ForwardingTypes::SendRecv>();

    if (rank == 0)
        std::cout << "ISend and IRecv: \n";

    time_test2<RedNBlackIterationsBlock, ForwardingTypes::ISendAndIRecv>();
