  ...
  void calculate_a(std::vector<body> &bodies_) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; ++j) {
        ...
        double r2 = (bodies_[i].r[0] - bodies_[j].r[0]) *
                        (bodies_[i].r[0] - bodies_[j].r[0]) +
                    (bodies_[i].r[1] - bodies_[j].r[1]) *
                        (bodies_[i].r[1] - bodies_[j].r[1]) +
                    (bodies_[i].r[2] - bodies_[j].r[2]) *
                        (bodies_[i].r[2] - bodies_[j].r[2]);

        double r = std::sqrt(r2);
        double tmp = std::max(r2 * r, eps);

        for (int k = 0; k < 3; ++k)
          bodies_[i].a[k] -=
              G * (bodies_[i].r[k] - bodies_[j].r[k]) * bodies_[j].mass / tmp;
      }
    }
  }
...
#pragma omp parallel for num_threads(nthreads)
  ...
  void logs_solve(double step = 0.1, double end_t = 20) {
    for (int i = 1; i < 5; ++i) {
    ...
    for (int i = 1; i < 5; ++i)
      files.push_back(
          std::ofstream("nt" + std::to_string(i) + ".txt", std::ios::app));
    ...
    for (size_t i = 0; i <= std::floor(end_t / step) + 1; i++) {
      ...
      if (i % 10000 == 0) {
        for (int i = 0; i < 4; ++i) {
          ...
          files[i] << std::setprecision(16) << bodies[i].r[0] << " "
                   << bodies[i].r[1] << " " << bodies[i].r[2] << "\n";
        }
      }
      ...
      for (body &bod : tmp_bodies) {
        for (int i = 0; i < 3; i++) {
          ...
          bodies[tmp].r[i] += step / 6. * bodies[tmp].v[i];

          bod.v[i] = y_n[tmp].v[i] + step / 2. * k1[1];
          bod.r[i] = y_n[tmp].r[i] + step / 2. * bod.v[i];
        }
      ...
      /*
                  for (body &bod : bodies) {
                      for (int i = 0; i < 3 ; i++) {
                          ...
                          bod.r[i] += step/6 * (k1[0] + 2*k2[0] + 2*k3[0] +
         k4[0]); bod.v[i] += step/6 * (k1[1] + 2*k2[1] + 2*k3[1] + k4[1]);
                      }
                  }
      */
    }
...
int main() {
  ...
  std::cout << std::setprecision(14) << N1.bodies[0].r[0] << " "
            << N1.bodies[2].v[0] << "\n";
